...
from litgpt.qat_prox2 import (
    ...
    QuantParamSelector, compute_dist_loss, compute_quantization_rate_fast,
)
import dataclasses
import math
import os
...
def fit(
    ...
    while state["step_count"] < max_steps:
        ...
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            f_loss = chunked_cross_entropy(logits[..., :-1, :], targets[..., 1:])  # task loss f(x)

            dist_loss = f_loss.new_zeros(())
            dist_dbg = None

            if qat.enabled:
                # dist term only once per optimizer step and disdbg just for logging
                if (not is_accumulating) and (gamma > 0.0):
                ...
                if qat.use_lagrange:
                    loss_total = dual_lambda * (f_loss - qat.beta) + (qat.dist_scale * gamma) * dist_loss
                else:
                    loss_total = f_loss + (qat.dist_scale * gamma) * dist_loss
            else:
                loss_total = f_loss

            fabric.backward(loss_total / accum_iters)

        running_loss.update(f_loss.detach())
        f_sum += float(f_loss.detach().item())
        ...
        if not is_accumulating:
            ...
            if qat.enabled and qat.use_lagrange:
                f_mean = f_sum / accum_iters
                dual_lambda = dual_ctl.step(f_mean)
